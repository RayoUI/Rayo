...
/// <summary>
/// A Rayo leaf control that embeds a live GPU-rendered 3D scene.
/// <para>
/// <see cref="VulkanScene3D"/> owns its own Vulkan device and renders the cube to an
/// off-screen RGBA8 image. Each animation tick the pixels are read back and
/// uploaded to the UI renderer via <see cref="IRenderer.CreateTextureFromPixels"/>,
/// making the control compatible with any renderer backend (Vulkan or SkiaSharp).
/// </para>
/// </summary>
public class VulkanView : View<VulkanView>, IFrameAnimation
{
    private VulkanScene3D? _scene;
    ...
    public override void Render(IRenderer renderer)
    {
        ...
        if (_cachedFrame != null && _cachedW > 0 && _cachedH > 0)
        {
            ...
            if (_fps > 0f)
            {
                string label   = $"Vulkan  {_fps:F0} FPS";
                float  pad     = 6f;
                float  fs      = 12f;
        ...
        renderer.DrawText("Initializing Vulkan scene...", cx - 110, cy - 8,
