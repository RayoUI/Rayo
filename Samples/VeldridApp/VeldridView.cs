...
/// <summary>
/// A Rayo leaf control that embeds a live Veldrid 3D scene (rotating cube).
/// <para>
/// <see cref="VeldridScene3D"/> owns its own Veldrid <c>GraphicsDevice</c> and renders
/// the cube to an off-screen RGBA8 texture on a dedicated background thread. Each
/// animation tick the pixels are read back and uploaded to the UI renderer via
/// <see cref="IRenderer.CreateTextureFromPixels"/>, making the control compatible
/// with any renderer backend (SkiaSharp, OpenGL, Vulkan).
/// </para>
/// </summary>
public class VeldridView : View<VeldridView>, IFrameAnimation
{
    private VeldridScene3D? _scene;
    ...
    public override void Render(IRenderer renderer)
    {
        ...
        if (_cachedFrame != null && _cachedW > 0 && _cachedH > 0)
        {
            ...
            if (_fps > 0f)
            {
                string label = $"Veldrid  {_fps:F0} FPS";
                float  pad   = 6f;
                float  fs    = 12f;
        ...
        renderer.DrawText("Initializing Veldrid scene...", cx - 115, cy - 8,
