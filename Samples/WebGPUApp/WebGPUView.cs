...
/// <summary>
/// A Rayo leaf control that embeds a live GPU-rendered 3D scene via WebGPU (wgpu-native).
/// <para>
/// <see cref="WebGPUScene3D"/> owns its own wgpu device and renders the cube to an
...
/// </para>
/// </summary>
public class WebGPUView : View<WebGPUView>, IFrameAnimation
{
    private WebGPUScene3D? _scene;
    ...
    public override void Render(IRenderer renderer)
    {
        ...
        if (_cachedFrame != null && _cachedW > 0 && _cachedH > 0)
        {
            ...
            if (_fps > 0f)
            {
                string label = $"WebGPU  {_fps:F0} FPS";
                float  pad   = 6f;
                float  fs    = 12f;
        ...
        renderer.DrawText("Initializing WebGPU scene...", cx - 115, cy - 8,
