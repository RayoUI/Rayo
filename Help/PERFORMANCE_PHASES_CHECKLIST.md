...
- [x] Evitar render recursivo completo cuando una rama no intersecta regiones sucias
- [x] Reducir limpieza global de dirty flags
- [x] Conectar `FrameScheduler` al pipeline real
- [x] Introducir una primera capa retenida para overlays/dialogs y root
...
- [x] Cache/pool de `SKPaint`
- [x] Cache de geometria/path frecuentes
- [x] Cache de medida y layout de texto
- [x] Reducir creacion temporal de `SKFont`
- [x] Eliminar LINQ y `ToArray()` en hot paths criticos
...
- `UITree` ya reutiliza texturas para el `root` y para cada overlay/dialog mediante `LayerCache`, invalida esas capas en relayout global, separa la invalidacion del `root` respecto a cambios internos de overlays y evita relayout completo del `root` cuando el trabajo pendiente pertenece solo a overlays.
- La entrega final de la Fase 2 divide `root` y overlays compatibles en una capa base mas capas retenidas por rama directa, de forma que los cambios locales rerasterizan solo la rama afectada y luego recomponen la salida sin recorrer de nuevo ramas limpias enteras.
- La primera entrega de la Fase 4 ya reutiliza `SKPaint` en rutas solidas de dibujo, outlines, paths sencillos y paints de texto/medicion dentro de `SkiaSharpRenderer`, reduciendo asignaciones nativas repetidas por frame.
- Tambien se cachean `SKFont` reutilizables por typeface/tamano/estilo y resultados de `MeasureTextWithFont(...)`, bajando el churn de fuentes temporales y mediciones repetidas en rendering y fallback de texto.
- El core ya evita la cadena `GetChildrenByZIndex() -> LINQ -> ToArray()` en render, hit testing y drag/drop: ahora genera snapshots estables sin LINQ y reutiliza listas ordenadas por `ZIndex` sin materializaciones extra por pasada.
